#ifndef CURVES_DYNAMIC_LIBRARY_H
#define CURVES_DYNAMIC_LIBRARY_H

#ifdef MY3DCURVESDLL_EXPORTS
#define API_CURVES_DYNAMIC_LIBRARY __declspec(dllexport)
#else
#define API_CURVES_DYNAMIC_LIBRARY __declspec(dllimport)
#endif 

#include <random>

class API_CURVES_DYNAMIC_LIBRARY TestDrive {
...
//struct API_CURVES_DYNAMIC_LIBRARY Point3D { double x{}, y{}, z{}; };               // 3D-Точка.
//struct API_CURVES_DYNAMIC_LIBRARY Vector3D { double x{}, y{}, z{}; };              // 3D-Вектор.

using namespace std;                                        //  Проект маленький, потому не так страшен - "using namespace" как его малюют )))

typedef API_CURVES_DYNAMIC_LIBRARY unsigned char Byte;

struct API_CURVES_DYNAMIC_LIBRARY Point_3D { double x{}, y{}, z{}; };                  // 3D-Точка.
struct API_CURVES_DYNAMIC_LIBRARY Vector_3D { double x{}, y{}, z{}; };                 // 3D-Вектор.

const double E = 1e-10;


//  Основной класс.
class API_CURVES_DYNAMIC_LIBRARY Curve_3D
{
protected:
    Point_3D C;
    Curve_3D() = default;
...
class API_CURVES_DYNAMIC_LIBRARY Circle_3D : public Curve_3D
{
    double Radius{};
...
class API_CURVES_DYNAMIC_LIBRARY Ellipse_3D : public Curve_3D
{
    double Radius_x = 0;
    double Radius_y = 0;
...
//  3D-спираль.
class API_CURVES_DYNAMIC_LIBRARY Spiral_3D : public Curve_3D
{
    double Radius = 0;
    double Step = 0;
...
class API_CURVES_DYNAMIC_LIBRARY Curve_Generator
{
...
#endif // CURVES_DYNAMIC_LIBRARY_H